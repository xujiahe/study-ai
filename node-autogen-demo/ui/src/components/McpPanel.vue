...
async function save() {
  ...
  await store.saveMcp(editing.value as McpServer & { name: string });
  editing.value = null;
}
    ...
    <div class="mcp-list">
      <div v-for="s in store.mcpServers" :key="s.id" class="mcp-item">
        ...
        <div class="mcp-actions">
          <button class="icon-btn" @click="startEdit(s)">✏️</button>
          <button class="icon-btn danger" @click="remove(s.id)">🗑️</button>
        </div>
      </div>
    </div>
