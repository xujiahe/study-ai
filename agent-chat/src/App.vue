    ...
    <main class="chat-area">
      <header class="topbar">
        ...
        <div class="topbar-right">
          <ModelSelector />
          <button class="clear-btn" @click="chat.clear()" title="Clear chat">Clear</button>
        </div>
      </header>
      ...
      <div class="messages" ref="messagesEl">
        <div v-if="chat.messages.length === 0" class="welcome">
          ...
          <div class="welcome-sub">Ask anything to get started</div>
          <div class="suggestions">
            <button
              v-for="s in ['What is RAG?', 'Write a binary search in TypeScript', 'Explain MCP protocol']"
      ...
      <div class="input-area">
        ...
        <div class="input-hint">
          <span v-if="chat.isStreaming" class="streaming-indicator">
            <span class="dot-pulse" /> Agents working...
          </span>
...
.tabs button { flex: 1; background: none; border: none; color: var(--text-muted); font-size: 12px; padding: 5px 8px; border-radius: 6px; cursor: pointer; }
...
.clear-btn { background: none; border: 1px solid var(--border); border-radius: 6px; cursor: pointer; font-size: 12px; padding: 4px 10px; color: var(--text-muted); }
...
.input-area { padding: 16px 20px; border-top: 1px solid var(--border); background: var(--bg); display: flex; flex-direction: column; gap: 6px; }
